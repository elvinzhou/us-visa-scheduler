...
登录和验证码识别均已全自动化——脚本使用您的账号密码，并通过 OpenAI GPT-4o-mini 自动完成验证码。为避免被网站检测为无头浏览器，仍需要一个可见的浏览器界面。请在有显示器的本地机器上运行：Windows 的 WSL2、树莓派，或任意带显示输出的 Ubuntu 系统。
...
# 签证网站账号密码
VISA_USERNAME=你的登录邮箱
VISA_PASSWORD=你的登录密码

# 安全问题答案（可选，仅当账号设置了安全问题时填写）
# SECURITY_A1=答案1
# SECURITY_A2=答案2
# SECURITY_A3=答案3

# OpenAI API Key — 用于自动识别登录验证码
OPENAI_API_KEY=sk-...
...
Chrome 窗口会在你的屏幕上打开，脚本将自动完成登录和验证码识别，然后进入循环监控。
...
1. 打开预约网站，通过 GPT-4o-mini 自动完成登录和验证码识别。
2. 进入监控循环：刷新页面 → 选择目标领事馆 → 通过拦截 API 响应获取可用日期（超时则回退到 DOM 解析）。
3. 与上一次快照对比，有变动则发送邮件。
4. 如果开启了自动预定，且发现符合日期范围的时间段，会自动走完预定流程。
5. 等待随机间隔（通常 3–5 分钟，每隔若干次检查后进入 7–9 分钟的较长休眠）后重复。
6. 如果检测到会话过期（跳转至登录页），自动重新登录，而不是静默重试。
